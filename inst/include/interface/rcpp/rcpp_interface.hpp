...
void forecast(int fyears) {
    std::shared_ptr<fims_info::Information<double> > info =
            fims_info::Information<double>::GetInstance();


    fims_info::Information<double>::population_iterator it;
    for (it = info->populations.begin(); it != info->populations.end(); ++it) {
        std::shared_ptr<fims_popdy::Population<double> > population = it->second;
        ...
        std::cout << "Population " << p.id << " biomass: ";
        for (int i = 0; i < p.biomass.size(); i++) {
            std::cout << p.biomass[i] << ", ";

        }

        std::cout << std::endl;
        std::cout << "Population " << p.id << " spawning biomass: ";
        for (int i = 0; i < p.spawning_biomass.size(); i++) {
            std::cout << p.spawning_biomass[i] << ", ";
        }

        std::cout << std::endl;
        for (int f = 0; f < p.fleets.size(); f++) {
            if (p.fleets[f]->is_survey) {
                std::cout << "Survey " << p.fleets[f]->id << " expected index: ";
            } else {
                std::cout << "Fleet " << p.fleets[f]->id << " expected index: ";
            }
            for (int k = 0; k < p.fleets[f]->expected_index.size(); k++) {
                std::cout << p.fleets[f]->expected_index[k] << ", ";
            }
            std::cout << std::endl;
        }
    }
...
RCPP_MODULE(fims) {
    ...
    Rcpp::function("forecast", forecast);
    ...
    Rcpp::class_<FleetInterface>("Fleet")
            ...
            .method("GetDerivedQuantities", &FleetInterface::GetDerivedQuantities);
    ...
    Rcpp::class_<PopulationInterface>("Population")
            ...
            .method("GetDerivedQuantities", &PopulationInterface::GetDerivedQuantities)
            .method("Forecast", &PopulationInterface::Forecast)
            .method("evaluate", &PopulationInterface::evaluate);
