...
public:
    ...
    Rcpp::List GetDerivedQuantities() {
        ...
        std::shared_ptr<fims_popdy::Fleet<double> > fleet = info->fleets[this->id];

        Rcpp::NumericVector index(fleet->catch_index.size());
        Rcpp::NumericVector catch_ (fleet->expected_index.size());
        for(size_t i =0; i < fleet->expected_index.size(); i++){
            index[i] = fleet->expected_index[i];
            catch_[i] = fleet->expected_catch[i];
        }
        
        results["index"] = index;
        results["catch"] =catch_;
